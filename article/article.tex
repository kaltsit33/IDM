...
\subsection{Model 1: relation to the LambdaCDM model}
...
\subsection{Quasar}
   ...
   which gives that
   \begin{equation}
      \log_{10}(F_X)=\beta+(\gamma-1)\log_{10}(4\pi)+\gamma\log_{10}(F_{UV})
                    +2(\gamma-1)\log_{10}(d_L),
   \end{equation}
   where $d_L$ is the luminosity distance same as which in the SNe Ia,Eq.15
